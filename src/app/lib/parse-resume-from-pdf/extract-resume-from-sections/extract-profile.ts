...
export const extractProfile = (sections: ResumeSectionToLines) => {
  ...
  const summarySectionText = summaryLines
    .flat()
    .map((textItem) => textItem.text)
    .join(" ");
  const objectiveLines = getSectionLinesByKeywords(sections, ["objective"]);
  const objectiveSectionText = objectiveLines
    .flat()
    .map((textItem) => textItem.text)
    .join(" ");

  // 将summary转换为数组
  let summaryText = summarySectionText || objectiveSectionText || summary;
  const summaryArray = summaryText ? [summaryText] : [];
