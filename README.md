<div align="center">
  <img src="public\logo-500.png" alt="ResumeToJob Logo" width="200" />
</div>

<div align="center">

# ResumeToJob | 简历制作网站

## [English](#english-version) | [简体中文](#chinese-version)

## 🔗

#### [网站无法访问？没 🪜，等了好久还是无法访问，想本地部署，看此小白教程](DEPLOYMENT_TUTORIAL.md)

#### [有 🪜，但还有问题？提交 Issue，帮助我们改进](https://github.com/ltlylfun/ResumeToJob/issues)

<p align="center">
  <a href="https://github.com/ltlylfun/ResumeToJob/stargazers">
    <img alt="GitHub stars" src="https://img.shields.io/github/stars/ltlylfun/ResumeToJob?style=for-the-badge&logo=github&color=FFB33A">
  </a>
  <a href="https://github.com/ltlylfun/ResumeToJob/network/members">
    <img alt="GitHub forks" src="https://img.shields.io/github/forks/ltlylfun/ResumeToJob?style=for-the-badge&logo=github&color=22BB88">
  </a>
  <a href="https://github.com/ltlylfun/ResumeToJob/issues">
    <img alt="GitHub issues" src="https://img.shields.io/github/issues/ltlylfun/ResumeToJob?style=for-the-badge&logo=github&color=FF6B6B">
  </a>
  <a href="https://github.com/ltlylfun/ResumeToJob/blob/main/LICENSE">
    <img alt="GitHub license" src="https://img.shields.io/github/license/ltlylfun/ResumeToJob?style=for-the-badge&logo=github&color=45AAF2">
  </a>
</p>

<div align="center">
  <img src="https://moe-counter.glitch.me/get/@ltlylfun-ResumeToJob?theme=rule34" alt="Moe Counter" />
</div>

</div>
<div align="center">
  <img src="https://github.com/user-attachments/assets/e63a39e1-af17-4ae7-8e42-1830e28864bf" alt="ResumeToJob Screenshot" width="800" />
</div>

---

<a id="chinese-version"></a>

## 简体中文

### 项目简介

ResumeToJob 是一个免费开源的在线简历制作工具，帮助求职者快速创建专业、美观的简历。支持多份简历管理，多种模板选择、实时编辑、PDF 导出等功能，同时保护您的隐私数据安全。

> ⚠️ **重要提示**：简历数据存储在浏览器本地，以防意外情况丢失，建议使用【管理简历】功能导出保存

<div align="center">
  <img src="https://github.com/user-attachments/assets/45260d58-2a93-45b2-a9ee-3635e0940df5" alt="Markdown支持演示" width="800" />
</div>

### 在线使用

访问 [https://resumetojob.ltlyl.fun/](https://resumetojob.ltlyl.fun/) 立即体验

### 许可证

本项目使用 [AGPL-3.0 license](LICENSE)。

### 致谢

[resumetojob](https://resumetojob.ltlyl.fun/) 是我在使用[open-resume](https://github.com/xitanggg/open-resume)的过程中为了满足我自己的需求而改造诞生的，感谢[open-resume](https://github.com/xitanggg/open-resume)的创建者和所有贡献者。

还有：

<p align="left">
  <a href="https://github.com/ltlylfun">
    <img src="https://avatars.githubusercontent.com/ltlylfun" width="40" alt="@ltlylfun" title="@ltlylfun" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/ASouwn">
    <img src="https://avatars.githubusercontent.com/ASouwn" width="40" alt="@ASouwn" title="@ASouwn" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/wegoof">
    <img src="https://avatars.githubusercontent.com/wegoof" width="40" alt="@wegoof" title="@wegoof" />
  </a>
</p>

---

<a id="english-version"></a>

## English

### Project Introduction

ResumeToJob is a free and open-source online resume builder that helps job seekers quickly create professional and beautiful resumes. It supports management of multiple resumes, multiple template options, real-time editing, PDF export, and always protects your privacy and data security.

> ⚠️ **Important Notice**: Resume data is stored in browser locally. To prevent accidental loss, it is recommended to use the [manage resumes] function to export and save

<div align="center">
  <img src="https://github.com/user-attachments/assets/b485c5e4-c0eb-459b-ad7f-9b06ec6447b2" alt="Markdown Support Demo" width="800" />
</div>

### Online Usage

Visit [https://resumetojob.ltlyl.fun/](https://resumetojob.ltlyl.fun/) to get started

### License

This project is licensed under the [AGPL-3.0 license](LICENSE).

### Acknowledgements

[ResumeToJob](https://resumetojob.ltlyl.fun/) was created as a result of my own needs while using [open-resume](https://github.com/xitanggg/open-resume). Many thanks to the creators and all contributors of [open-resume](https://github.com/xitanggg/open-resume).

and:

<p align="left">
  <a href="https://github.com/ltlylfun">
    <img src="https://avatars.githubusercontent.com/ltlylfun" width="40" alt="@ltlylfun" title="@ltlylfun" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/ASouwn">
    <img src="https://avatars.githubusercontent.com/ASouwn" width="40" alt="@ASouwn" title="@ASouwn" />
  </a>
  &nbsp;&nbsp;
  <a href="https://github.com/wegoof">
    <img src="https://avatars.githubusercontent.com/wegoof" width="40" alt="@wegoof" title="@wegoof" />
  </a>
</p>
